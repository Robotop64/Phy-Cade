...
void Scene::MainMenu()
{
    ...
    while (!WindowShouldClose() && !close)
    {
        ...
        // Log::msg("Window", "Layout calculated.");
...
void myLayout(ClayMan &clayMan)
{
    ...
    clayMan.openElement({.id = clayMan.hashID("OuterContainer"),
                         .layout = {
    ...
    { // Children of OuterContainer are in this block. The block is not necessary, but helps with structure.
        ...
        clayMan.element(
            {.id = clayMan.hashID("AnotherElement"),
             ...
             .border = {.color = {123, 123, 0, 255}, .width = {5, 5, 5, 5, 0}}},
            [&]
            {
                clayMan.element(
                    {.layout = {
