        ...
        tiles.forEach((vec, tile) ->
        {
            if (tile.type == Type.playerSpawn) {
                gameState.gameObjects.add(
                        new PacmanObject((int) (this.tileSize * 2. / 3.), vec.multiply(this.tileSize).add(new Vector2d().cartesian(4, 4)), gameState, gameState.player)
                );
            }
