...
ClayMan::ClayMan(
    ...
    void *measureTextUserData) : windowWidth(initialWidth), windowHeight(initialHeight)
{
    if (windowWidth == 0)
    {
        windowWidth = 1;
    ...
    Clay_Initialize(clayMemory, (Clay_Dimensions){.width = (float)windowWidth, .height = (float)windowHeight}, (Clay_ErrorHandler)handleErrors);
